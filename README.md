...
- **`site_wavelet_plots/`**  
  Output figures for **Sites 1–5** as described in the manuscript.  
...
- **`sliced_csvs/`**  
  ...
  These files are generated by `pixel_centers.ipynb` and are used to parallelise the wavelet analysis for use on a high performance computer (HPC).
...
- **`individual-site-wavelet-analysis.ipynb`**  
  Demonstrates wavelet analysis for selected sites (Sites 1–5) used in the manuscript.
...
- **`adding-white-noise.ipynb`**  
  Error propagation analysis: script performs a Monte Carlo analysis of Gaussian white noise to assess the effect of measurement uncertainty on wavelet results.
...
- **`power-analysis-timeseries-complete-scales_windows.py`**  
  Runs the wavelet power analysis for each latitude–longitude coordinate listed in the pixel centroid CSV files. This was run on a HPC as 300 parallel jobs (each job running a subset of global coordinates).

- **`submit-complete-scales_windows.sh`**  
  Example of HPC submission script (using Portable Batch System for scheduling) to run the wavelet analysis as an array job (e.g., 300 parallel jobs).
...
3. Wavelet analysis can be tested on Sites 1–5 using the notebook `individual-site-wavelet-analysis.ipynb`. Alternatively, a self-contained demonstration of the methodology is provided in `minimal-working-example-site5-wa.ipynb`, which does not require completing steps 1 and 2.
